...
<br>Нюансы по дз. В пакете **version_02** пытался реализовать код показанный на уроке, но выходит так что потоки отрабатывают друг за другом свои условия. Т.е. отработал полностью 1 поток, потом полностью 2 поток, потом полность. 3 поток. В целом как и должен работать метод _join()_. Поэтому другого способа решения задачи как представленн в пакете **version_01** не вижу.В пакете **version_01** код отрабатывает согласно условию задачи.  
<br>И по необязательной части.С задачей не справился. Пытался реализовать вводя вторую переменную, но в итоге все равно не реализовал вывод данных как по условию. Можешь хотябы посмотреть, и подсказать ход мысли правильный или нет? Сейчас выпадает реализация 3 потока выпадает. Пытался и через _yield()_ реалиовать. Но как то все слишком рандомно получается.
